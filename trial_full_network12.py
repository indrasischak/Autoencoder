...
traindata = np.zeros(shape=[total_train_row,num_input+2*num_derv])
...
for ijjjj in range(16,16+1):
    mnistx=mnist[:,2*num_derv:num_input+2*num_derv]
    mnistx=mnistx.reshape([total_train_row,num_input])
    #mnisty = np.zeros(shape=[total_train_row,2])
    mnisty=mnist[:,ijjjj-1]
    mnisty=mnisty.reshape([total_train_row,1])
    mnistyy=mnist[:,ijjjj-1+num_derv]
    mnistyy=mnistyy.reshape([total_train_row,1])
    mnisty_new=np.concatenate((mnisty, mnistyy), axis=1)
# Store layers weight & bias
    weights = {
    'h'+str(ijjjj): tf.Variable(tf.truncated_normal([n_input, n_hidden_1], stddev=0.1)),
    'out'+str(ijjjj): tf.Variable(tf.truncated_normal([n_hidden_1, 2], stddev=0.1))
    }

    biases = {
    'b'+str(ijjjj): tf.Variable(tf.constant(0.1, shape=[n_hidden_1])),
    'out'+str(ijjjj): tf.Variable(tf.constant(0.1, shape=[2]))
    }
...
# Minimize the mean squared errors.
    ...
    loss=tf.reduce_mean(tf.squared_difference(y_data[:,0], pred[:,0]))+tf.reduce_mean(tf.squared_difference(y_data[:,1], pred[:,1]))
    #loss=tf.nn.l2_loss(y_data,pred)
...
# Launch the graph.
    ...
    for step in range(training_epochs):
        ...
        #print("TRAIN:", train_index, "TEST:", test_index)
            ...
            #trainy=mnist[:,1]
                ...
                #X_train, X_test = mnistx[train_index],mnistx[test_index]
                #y_train, y_test = mnisty[train_index], mnisty[test_index]
            sess.run(train, feed_dict={x_data: mnistx, y_data: mnisty_new})

            curLoss1 = sess.run(loss, feed_dict={x_data: mnistx, y_data: mnisty_new})
    ...
    #error=z1-mnisty
    data={'W1':w1,'W2':w2,'b1':b1,'b2':b2,'pred':z1,'act':mnisty}
