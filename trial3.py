...
sheet3 = workbook3.add_sheet('fault_predictioon')
...
def model_fn(features, targets, mode, params):
  ...
  output_layer = tf.contrib.layers.fully_connected(inputs=second_hidden_layer,num_outputs=1,activation_fn=tf.nn.sigmoid)
  ...
  rate = tf.train.exponential_decay(0.0001, step, 1, 0.000001)
...
def main(unused_argv):
  ...
  j=0
  k=0
  sheet3.write(0,0,'NN prediction')
  sheet3.write(0,1,'Actual data')
  sheet3.write(0,2,'Difference')
  sheet3.write(0,3,'Presence of fault')
  for i, p in enumerate(predictions):
    #print(p["disturbance"])
    sheet3.write(j+1,0,p["disturbance"])
    sheet3.write(j+1,1,float(data[j][9]))
    sheet3.write(j+1,2,abs(float(data[j][9])-p["disturbance"]))
    
    if abs(float(data[j][9])-p["disturbance"])>ev["rmse"]+0.2:
      sheet3.write(j+1,3,'YES')
      k=k+1
    else:
      sheet3.write(j+1,3,'NO')
    j=j+1
  print("Fault Detection Successful!!!")
  workbook3.save('fault_prediction.xls')
...
if __name__ == "__main__":
  ...
  #saver.save(sess, 'C:/Users/chak282/AppData/Local/Programs/Python/myModel.ckpt')
    #saver.export_meta_graph('./myModel.meta')
